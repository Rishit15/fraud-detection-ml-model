...
function renderAdmin(data) {
  ...
  const pendingData = data.filter(item => item.status === "Pending" || item.status === "Confirmed");
  ...
  const totalConfirmed = data.filter(d => d.status === "Confirmed").length;
...
document.addEventListener("DOMContentLoaded", async function() {
  ...
  console.log("📊 Rendering admin with", data.length, "items");
  renderAdmin(data);
