...
window.API_BASE_URL = "http://127.0.0.1:5000/api";
  ...
  function renderDashboard(data) {
    ...
    const pending = data.filter(data => data.status === "Pending").length;
    const confirmed = data.filter(data => data.status === "Confirmed").length;
    const rejected = data.filter(data => data.status === "Rejected").length;

    // Update KPI cards - safe null checks
    const kpis = [
      { id: "kpi-transfers", value: total },
      { id: "kpi-abnormal", value: confirmed },
